...
export function DragonTigerPage({ onSelectStock }: { onSelectStock?: (tsCode: string) => void } = {}) {
                                ...
                                {data.map((item) => (
                                    ...
                                    >
                                        <td className="px-3 py-2 text-center">
                                            <button
                                                type="button"
                                                className="group inline-flex flex-col items-center rounded-md px-2 py-1 transition-colors hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/60"
                                                onClick={() => onSelectStock?.(item.ts_code)}
                                                disabled={!onSelectStock}
                                                aria-label={`查看${item.name}个股详情`}
                                            >
                                                <span className="font-medium text-foreground text-sm transition-colors group-hover:text-blue-600">{item.name}</span>
                                                <span className="text-xs text-muted-foreground transition-colors group-hover:text-blue-500">{item.ts_code}</span>
                                            </button>
                                        </td>
